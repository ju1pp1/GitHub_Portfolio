...
app.get('/status', async (_req, res) => {
  try {
    const line1 = `Timestamp1:${isoUtcNow()} status: uptime ${uptimeHours()} hours, free disk in root: ${freeDiskMB()} MBytes`;
    await postToStorage(line1);

    http.get(`${SERVICE2_URL}/status`, (r2) => {
      let body = ''; r2.on('data', (c) => body += c);
      r2.on('end', () => res.type('text/plain').send(`${line1}\n${body.trim()}`));
    }).on('error', (e) => res.status(502).type('text/plain').send(`Service2 error: ${e.message}`));
  } catch (e) {
...
app.get('/log', (_req, res) => {
  http.get(`${STORAGE_URL}/log`, (r) => { res.status(r.statusCode || 200); res.type('text/plain'); r.pipe(res); })
      .on('error', (e) => res.status(502).type('text/plain').send(`Storage error: ${e.message}`));
});

app.get('/', (_req, res) => res.type('text/plain').send('OK. Try /status or /log'));
