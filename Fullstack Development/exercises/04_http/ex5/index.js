...
http.createServer(function (request, response) {
  ...
  //if (request.headers['accept'].includes('application/json')) {
    ...
    //check if accept header matches one of the Mime types
    let acceptHeader = request.headers['accept'];
    for(let mimeType in mimeTypes) {
      if(acceptHeader && acceptHeader.includes(mimeType)) {
        const fileName = mimeTypes[mimeType];
        readFileSendResponse(fileName, mimeType, response);
        return;
      }
    }
