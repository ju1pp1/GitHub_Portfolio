 ...
 */
const drawArrows = (actors, timeout, drawArrow, i = 0, forward = true) => {

  if(forward) {
    drawArrow(i, timeout, actors.length - 1);
    setTimeout(() => {
      drawArrows(actors, timeout, drawArrow, i + 1, forward);
    }, timeout);
  } else {
  ...
  if(i === actors.length - 1) {
    setTimeout(() => {
      drawArrows(actors, timeout, drawArrow, i - 1, false);
    }, timeout);
  }
  /*
if(i >= actors.length - 1) return;

//Recursively call drawArrows
drawArrow(i, i + 1, actors.length - 1);
